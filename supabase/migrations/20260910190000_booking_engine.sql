...
-- Atomically reserves seats. The row lock prevents overbooking under concurrency.
...
BEGIN
  ...
  INSERT INTO public.reservations (
    availability_id, agency_id, user_id, status, total_price,
    commission_amount, passenger_count, lead_passenger_name,
    lead_passenger_document, expires_at
  ) VALUES (
    p_availability_id, p_agency_id, v_user, 'confirmed', v_total,
    v_commission, p_passenger_count, trim(p_lead_passenger_name),
    NULLIF(trim(p_lead_passenger_document), ''), NULL
  ...
  VALUES (
    v_user, 'reservation.created', 'reservations', v_reservation,
    jsonb_build_object('availability_id', p_availability_id, 'passenger_count', p_passenger_count, 'total_price', v_total)
  );
...
BEGIN
  ...
  IF v_role NOT IN ('admin', 'operator') AND v_res.user_id <> v_user THEN
  ...
  VALUES (v_user, 'reservation.cancelled', 'reservations', p_reservation_id,
    jsonb_build_object('status', v_res.status), jsonb_build_object('status', 'cancelled', 'passenger_count', v_res.passenger_count));
