...
-- Confirmed reservations and R3 voucher redemption remain unchanged.
...
ALTER TABLE public.reservations
  ...
  CHECK (status IN ('pending', 'held', 'confirmed', 'completed', 'cancelled', 'expired'));

CREATE UNIQUE INDEX IF NOT EXISTS idx_reservations_user_idempotency_key
  ON public.reservations(user_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_reservations_active_holds
  ON public.reservations(availability_id, expires_at)
  WHERE status = 'held';

-- This helper is only called from SECURITY DEFINER mutation RPCs after the
-- availability row has been locked. It releases seats and records expiry.
CREATE OR REPLACE FUNCTION public.release_expired_holds(p_availability_id UUID)
RETURNS INT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    SET available_seats = LEAST(total_seats, available_seats + v_hold.passenger_count),
        updated_at = NOW()
    ...
    VALUES (
      ...
      'reservations',
      v_hold.id,
      jsonb_build_object('status', 'held', 'passenger_count', v_hold.passenger_count),
      jsonb_build_object('status', 'expired', 'availability_id', p_availability_id)
    );
...
CREATE OR REPLACE FUNCTION public.create_reservation_hold(
  ...
  p_hold_seconds INT DEFAULT 900
)
RETURNS TABLE (
  reservation_id UUID,
...
BEGIN
  ...
  IF v_role = 'agency' AND NOT EXISTS (
    ...
    WHERE au.agency_id = p_agency_id AND au.user_id = v_user
  ) THEN
    RAISE EXCEPTION 'AGENCY_NOT_ALLOWED';
  END IF;

  v_fingerprint := md5(CONCAT_WS('|',
    p_availability_id::TEXT,
    p_agency_id::TEXT,
    p_passenger_count::TEXT,
    BTRIM(p_lead_passenger_name),
    NULLIF(BTRIM(p_lead_passenger_document), '')
  ));

  -- Lock order starts at availability and is shared by expiry/confirmation.
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT
      ...
      v_availability.available_seats,
  ...
  INSERT INTO public.reservations (
    ...
    passenger_count, lead_passenger_name, lead_passenger_document, expires_at,
    idempotency_key, request_fingerprint
  ) VALUES (
    p_availability_id, p_agency_id, v_user, 'held', v_total, v_commission,
    p_passenger_count, BTRIM(p_lead_passenger_name), NULLIF(BTRIM(p_lead_passenger_document), ''),
    v_expires_at, BTRIM(p_idempotency_key), v_fingerprint
  ...
  RETURN QUERY SELECT
    v_reservation, 'held'::TEXT, v_expires_at,
    v_availability.available_seats - p_passenger_count,
    v_total, v_commission, FALSE;
END;
$$;
...
BEGIN
  ...
  SELECT public.current_user_role() INTO v_role;
  IF v_role NOT IN ('admin', 'agency') THEN RAISE EXCEPTION 'ROLE_NOT_ALLOWED'; END IF;
  IF v_role = 'agency' AND NOT (v_res.user_id = v_user AND EXISTS (
    SELECT 1 FROM public.agencies_users au
    WHERE au.agency_id = v_res.agency_id AND au.user_id = v_user
  )) THEN
  ...
  IF v_res.expires_at IS NULL OR v_res.expires_at <= NOW() THEN
  ...
  VALUES (
    ...
    'reservations',
    v_res.id,
    jsonb_build_object('status', 'held', 'expires_at', v_res.expires_at),
    jsonb_build_object('status', 'confirmed', 'voucher_issued', TRUE)
  );
...
COMMENT ON FUNCTION public.create_reservation_hold(UUID, UUID, INT, TEXT, TEXT, TEXT, INT)
IS 'Creates an expiring, idempotent inventory hold. It does not issue a voucher or represent payment.';
COMMENT ON FUNCTION public.confirm_reservation_hold(UUID)
IS 'Confirms an unexpired owned hold and issues its voucher atomically.';
