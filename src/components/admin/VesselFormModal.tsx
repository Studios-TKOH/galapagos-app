...
export function VesselFormModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal */}
      <div role="dialog" aria-modal="true" aria-labelledby="vessel-modal-title" className="relative w-full max-w-xl max-h-[90vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-bottom-2 duration-300 ease-out flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
          <h2 id="vessel-modal-title" className="text-xl font-bold text-slate-900 dark:text-white">Nueva Embarcación</h2>
          <button 
            type="button"
            onClick={onClose}
            aria-label="Cerrar formulario de embarcación"
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <select id="vessel-status" className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all">
