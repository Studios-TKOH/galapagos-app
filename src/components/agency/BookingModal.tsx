...
import { X, Users, CreditCard, Clock, Loader2, CheckCircle2 } from "lucide-react";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="booking-modal-title" className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
          <div><h2 id="booking-modal-title" className="text-xl font-bold text-slate-900 dark:text-white">Confirmar reserva</h2><p className="text-sm text-slate-500 mt-1">{tour.boatName} · {tour.routeName}</p></div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          {success ? (
            ...
            <>
              <div className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                <Users className="w-5 h-5 text-primary" />
                <div className="flex-1"><p className="font-bold">Pasajeros</p><p className="text-sm text-slate-500">Disponibles: {tour.availableSeats}</p></div>
                <div className="flex items-center gap-3"><button type="button" onClick={() => setPassengers(Math.max(1, passengers - 1))} className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 font-bold">−</button><span className="w-6 text-center font-bold">{passengers}</span><button type="button" onClick={() => setPassengers(Math.min(tour.availableSeats, passengers + 1))} className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 font-bold">+</button></div>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <input value={leadPassenger} onChange={e => setLeadPassenger(e.target.value)} placeholder="Nombre completo del titular" className="px-4 py-3 rounded-xl border bg-slate-50 dark:bg-slate-800" />
                <input value={document} onChange={e => setDocument(e.target.value)} placeholder="Pasaporte / Cédula" className="px-4 py-3 rounded-xl border bg-slate-50 dark:bg-slate-800" />
              </div>
              <div className="p-5 rounded-2xl bg-blue-50 dark:bg-blue-900/10 space-y-3">
                <div className="flex justify-between"><span>Precio público</span><span>${publicTotal.toFixed(2)}</span></div>
                <div className="flex justify-between text-emerald-600"><span>Comisión (15%)</span><span>- ${commissionAmount.toFixed(2)}</span></div>
                <div className="pt-3 border-t flex justify-between font-black"><span>Neto</span><span className="text-2xl text-primary">${netTotal.toFixed(2)}</span></div>
              </div>
