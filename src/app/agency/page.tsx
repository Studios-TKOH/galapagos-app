...
import { Search, Calendar, MapPin, Users, Filter, Clock, Ship, CheckCircle2, Loader2, ArrowRight } from "lucide-react";
...
type AvailabilityRow = {
  id: string; date: string; departure_time: string; total_seats: number; available_seats: number; status: string;
...
function formatDuration(minutes?: number) { if (!minutes) return "Duración no especificada"; const days = Math.floor(minutes / 1440); const hours = Math.floor((minutes % 1440) / 60); if (days) return `${days} día${days > 1 ? "s" : ""}${hours ? ` / ${hours} h` : ""}`; return `${hours} h`; }

export default function AgencySearchPage() {
  const [rows, setRows] = useState<AvailabilityRow[]>([]); const [selectedTour, setSelectedTour] = useState<BookingTour | null>(null);
  const [dateSearch, setDateSearch] = useState(""); const [destination, setDestination] = useState(""); const [passengers, setPassengers] = useState(2);
  ...
  async function searchAvailability() {
    setLoading(true); setError(""); setSearched(true); const isoDate = toIsoDate(dateSearch);
    if (dateSearch && !isoDate) { setError("La fecha no es válida. Usa DD/MM/AAAA."); setRows([]); setLoading(false); return; }
    let query = supabase.from("availability").select("id,date,departure_time,total_seats,available_seats,status,vessel:vessels(name),route:routes(name,origin,destination,duration_minutes),tour:tours(name,description,base_price)").eq("status", "active").gt("available_seats", 0).order("date", { ascending: true }).order("departure_time", { ascending: true }).limit(50);
    if (isoDate) query = query.eq("date", isoDate);
    const { data, error: queryError } = await query;
    if (queryError) { setError("No se pudo consultar la disponibilidad. Revisa la conexión con Supabase."); setRows([]); }
    else { const normalized = (data ?? []) as unknown as AvailabilityRow[]; const needle = destination.trim().toLowerCase(); setRows(needle ? normalized.filter(row => `${row.route?.name ?? ""} ${row.route?.origin ?? ""} ${row.route?.destination ?? ""}`.toLowerCase().includes(needle)) : normalized); }
    setLoading(false);
  }
  ...
  return <div className="w-full">
    <div className="relative pt-16 pb-32 flex flex-col items-center justify-center text-center px-4 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl space-y-6">
        ...
        <div className="mt-8 bg-white dark:bg-slate-900 rounded-3xl p-3 shadow-2xl ring-1 ring-white/10 flex flex-col md:flex-row items-center gap-3 w-full">
          ...
          <div className="w-full md:w-40 flex items-center px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-2xl transition-all duration-300 focus-within:ring-2 focus-within:ring-primary/30"><Users className="w-5 h-5 text-primary mr-3 shrink-0" /><input type="number" aria-label="Número de pasajeros" value={passengers} onChange={e => setPassengers(Math.max(1, Number(e.target.value) || 1))} min="1" max="100" className="bg-transparent border-none outline-none w-full text-slate-900 dark:text-white font-medium" /></div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 relative z-20 pb-20">
      <div className="flex items-center justify-between gap-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur p-4 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 mb-8">
        ...
        <button type="button" onClick={searchAvailability} className="min-h-11 flex items-center gap-2 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all duration-300 active:scale-95"><Filter className="w-4 h-4" /> Actualizar</button>
      </div>
      ...
      {loading ? <div className="py-20 text-center"><Loader2 className="w-10 h-10 mx-auto animate-spin text-primary" /><p className="mt-3 text-slate-500 dark:text-slate-400">Consultando disponibilidad…</p></div> : searched && rows.length === 0 ? <div className="py-20 px-6 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm"><Ship className="w-10 h-10 mx-auto text-slate-300 dark:text-slate-600" /><p className="mt-4 font-bold text-slate-900 dark:text-white">No hay salidas con esos criterios.</p><p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Prueba otra fecha, ruta o destino.</p></div> : <div className="space-y-5">{rows.map(row => { const tourName = row.tour?.name || row.route?.name || "Salida"; const seats = row.available_seats || 0; const price = Number(row.tour?.base_price || 0); return <div key={row.id} className="group bg-white dark:bg-slate-900 rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-sm hover:shadow-2xl hover:-translate-y-1 border border-slate-200/70 dark:border-slate-800/70 transition-all duration-300 ease-out">
        ...
        <div className="flex-1 p-6 flex flex-col"><div className="flex items-center gap-2 text-primary font-bold text-sm mb-2 uppercase tracking-wide"><Ship className="w-4 h-4" />{row.vessel?.name || "Embarcación"}</div><h3 className="text-2xl font-black text-slate-900 dark:text-white mb-2 leading-tight">{tourName}</h3><p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{row.route?.origin || "Origen"} → {row.route?.destination || "Destino"}</p><div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-600 dark:text-slate-400 font-medium"><span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> {formatDuration(row.route?.duration_minutes)}</span><span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600" /><span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> {formatDate(row.date)} · {row.departure_time?.slice(0,5)}</span></div><div className="mt-auto pt-5"><span className="inline-flex px-3 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 text-sm font-bold border border-emerald-100 dark:border-emerald-900/50">{seats} cupo{seats === 1 ? "" : "s"} libre{seats === 1 ? "" : "s"}</span></div></div>
        <div className="w-full md:w-64 bg-slate-50 dark:bg-slate-800/50 p-6 flex flex-col justify-center items-center md:items-end text-center md:text-right border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-800"><p className="text-sm text-slate-500 dark:text-slate-400">Precio por persona</p><p className="text-3xl font-black text-slate-900 dark:text-white my-2">{price ? `$${price.toFixed(2)}` : "Consultar"}</p><p className="text-xs text-emerald-600 dark:text-emerald-400 font-bold mb-6">Comisión de agencia: 15%</p><button type="button" disabled={!price || seats < passengers} onClick={() => setSelectedTour({ id: row.id, boatName: row.vessel?.name || "Embarcación", routeName: tourName, price, availableSeats: seats })} className="group/btn w-full py-3.5 bg-slate-900 hover:bg-slate-800 dark:bg-primary dark:hover:bg-blue-600 text-white font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 ease-in-out hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2">{seats < passengers ? "Cupos insuficientes" : <>Reservar / Bloquear <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover/btn:translate-x-1" /></>}</button></div>
      </div>; })}</div>}
    </div>
    {selectedTour && <BookingModal key={selectedTour.id} isOpen onClose={() => setSelectedTour(null)} tour={selectedTour} initialPassengers={passengers} />}
  </div>;
}
