...
import { Plus, Mail, MessageCircle, MoreVertical, Edit2, ShieldBan, MapPin, Building2, CheckCircle2 } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pt-4">
        {mockAgencies.map((agency) => (
          <div 
            key={agency.id} 
            className="glass rounded-2xl flex flex-col transition-all hover:shadow-xl dark:hover:shadow-primary/5 border border-slate-200/60 dark:border-slate-800/60 overflow-hidden relative"
          >
            {/* Cabecera Tarjeta */}
            ...
            <div className="p-6 flex-1 flex flex-col gap-4">
              ...
              <div className="mt-auto pt-4 grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800/60">
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-1">Comisión</p>
                  <p className="text-lg font-bold text-slate-900 dark:text-white">{agency.commission}%</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-1">Mes Actual</p>
            ...
            <div className="grid grid-cols-2 divide-x divide-slate-100 dark:divide-slate-800 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20">
              <button className="flex items-center justify-center gap-2 py-3 text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-primary transition-colors">
                <Edit2 className="w-4 h-4" />
                Editar
              </button>
              <button className={`flex items-center justify-center gap-2 py-3 text-sm font-medium transition-colors ${
                agency.status === 'active' 
