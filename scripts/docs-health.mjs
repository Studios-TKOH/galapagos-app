...
const discovered=[];
if(exists('src/app/page.tsx')) discovered.push('src/app');
...
const stale=[];
for(const m of modules){ if(!exists(m.doc)||!exists(m.modulePath)) continue; const code=gitTs(m.modulePath); const doc=gitTs(m.doc); if(code>doc) stale.push({...m,code,doc}); }
